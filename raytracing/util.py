...
def radix_sort_binary(
    ...
    """
    Sorts a slice of an array in the range [start, end) using radix sort.

    The sort is performed based on integer keys returned by `value_func` for
    each element. The keys are treated as `total_bits` unsigned integers. The
    sort proceeds in passes, examining `bits_per_pass` from the least
    significant to the most significant bits.
    """
